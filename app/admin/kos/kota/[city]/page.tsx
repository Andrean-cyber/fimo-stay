// app/admin/kos/kota/[city]/page.tsx
import { prisma } from '@/lib/prisma'
import { requireAdmin } from '@/utils/auth/require-admin'
import Link from 'next/link'
import { AlertTriangle, Home, MapPin, User, Search, ChevronLeft } from 'lucide-react'
...
export default async function KosByCityPage({
  params,
  searchParams,
}: {
  ...
  const buildHref = (p: number) => {
    ...
    return `/admin/kos/kota/${encodeURIComponent(cityName)}?${params.toString()}`
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-1 flex items-center gap-2 text-xl font-bold text-fimo-navy sm:text-2xl">
          <MapPin className="h-5 w-5" />
          {cityName}
        </h1>
        <p className="mt-1 text-xs text-gray-500 sm:text-sm">
          {total === 0 ? '0 kos' : `Menampilkan ${start}–${end} dari ${total} kos`}
        </p>
      </div>

      <form
        action={`/admin/kos/kota/${encodeURIComponent(cityName)}`}
        method="get"
      ...
      >
        <div className="relative flex-1">
          ...
          <input
            ...
            name="q"
            defaultValue={query}
            placeholder={`Cari nama atau owner di ${cityName}...`}
      ...
      {daftarKos.length === 0 ? (
        <div className="rounded-2xl border border-fimo-gray bg-white shadow-sm">
          <EmptyState
            icon={Home}
            title={query ? 'Kos tidak ditemukan' : `Belum ada kos di ${cityName}`}
            description={
              query
                ? `Tidak ada kos yang cocok dengan "${query}" di ${cityName}.`
                : 'Mulai dengan menambahkan kos pertama di kota ini.'
            }
            actionLabel={query ? undefined : '+ Tambah Kos'}
            actionHref={query ? undefined : `/admin/kos/new?city=${encodeURIComponent(cityName)}`}
          />
        </div>
      ) : (
        <>
          ...
          <div className="space-y-3 md:hidden">
            {daftarKos.map((kos) => {
              ...
              return (
                <div key={kos.id} className="rounded-2xl border border-fimo-gray bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span
                      className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ${
                        isStale ? 'bg-red-50 text-red-600' : isHiddenManual ? 'bg-fimo-gray text-gray-600' : 'bg-green-50 text-green-700'
                      }`}
                    >
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-fimo-gray pt-3">
                    ...
                    <div className="flex items-center gap-4">
                      <Link href={`/admin/kos/${kos.id}/edit`} className="text-sm font-medium text-fimo-navy hover:text-fimo-blue">
                        Edit
                      </Link>
                    ...
                    <tr key={kos.id} className="transition-colors hover:bg-fimo-gray/30">
                      ...
                      <td className="whitespace-nowrap px-5 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-3">
                          <Link href={`/admin/kos/${kos.id}/edit`} className="text-sm font-medium text-fimo-navy hover:text-fimo-blue lg:text-[15px]">
                            Edit
                          </Link>
                          <ConfirmDeleteButton action={deleteKos.bind(null, kos.id)} itemName={kos.name} />
                        </div>
